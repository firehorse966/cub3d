...
/*   Updated: 2024/09/25 10:36:48 by aiturria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cb_wallcheckhorz(t_game *game, float x, float y)
{
	...
	if (game->map->map2d[mapy - 1][mapx] == '1'
		...
		return (0);
	else if (game->map->map2d[mapy - 1][mapx] == 'D'
		|| game->map->map2d[mapy][mapx] == 'D')
	{
		game->ray->doorx = 1;
		return (0);
	}
...
int	cb_wallcheckvert(t_game *game, float x, float y)
{
	int	mapx;
	int	mapy;
	...
	if (game->map->map2d[mapy][mapx - 1] == '1'
		...
		|| game->map->map2d[mapy][mapx] == ' ')
		return (0);
	else if (game->map->map2d[mapy][mapx - 1] == 'D'
		|| game->map->map2d[mapy][mapx] == 'D')
	{
		game->ray->doory = 1;
		return (0);
...
double	cb_gethorizontal(t_game *game, double rayangle)
{
	...
	while (cb_wallcheckhorz(game, first_x, first_y))
	{
		first_x += x_step;
		first_y += y_step;
	}
...
double	cb_getvertical(t_game *game, double rayangle)
{
	...
	while (cb_wallcheckvert(game, first_x, first_y))
	{
		first_x += x_step;
		first_y += y_step;
	}
